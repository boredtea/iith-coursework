...
//globally declaring the graph and status arrays to deal with the memory issues
//List **Graph;
List *Graph[10000000];
...
void addEdge(List *L, int vertex)
{
    ...
    {
        int flag = 0; //checks if node was inserted or not
        ...
        while (Q->length > 0)
        {
            ...
            for (int i = 1; i < Graph[popped]->vertices; i++)
            {
                ...
                if (status[current->vertex] == -1)
                {
                    ...
                    status[current->vertex] = 0; //satus 1: vertex has been visited
                }
            }
        }
    }
...
int main()
{
    ...
    //Graph = (List **)malloc(sizeof(List *) * numberOfVertices);
    //status = (int *)malloc(sizeof(int) * numberOfVertices);
    ...
    while (scanf(" %c", &choice) != -1)
    {
        ...
        else if (choice == 'B')
        {
            ...
            //memset(status, -1, sizeof(int) * numberOfVertices);
