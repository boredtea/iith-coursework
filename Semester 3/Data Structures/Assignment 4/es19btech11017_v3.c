...
/*
Function to free all memory from a list.
*/
void freeList(List *L)
{
    Node *current = L->start, *temp;
    for(int i = 1; i < L->vertices; L++)
    {
        temp = current;
        current = current->next;
        free(temp);
...
void BFS(List *Graph[], int status[], Queue *Q, int startVertex)
{
...
int main()
{
    ...
    while (scanf(" %c", &choice) != -1)
    {
        ...
        else if (choice == 'B')
        {
            ...
            {
                BFS(Graph, status, Q, startVertex);
                printf("\n");
            }
        }
    }
