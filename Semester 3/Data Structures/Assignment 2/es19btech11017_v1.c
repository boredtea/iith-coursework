...
void Successor(Tree *T, char key[7])
{
    Node *root = T->start;
    Node *successor = NULL;
    Node *lastParent; //if the key is not in the BST, this points to what would have been it's parent if it was inserted into the BST
    int flag = 0;
    ...
    while(root != NULL)
    {
        ...
        if(compare(key, root->key) == 2) //case where right subtree exists
        {
            if(root->rightChild != NULL)
            {
                ...
                flag = 1; //key is present in the BST and it has a right child
            }
            break;
        }
    ...
    if(flag == 0) //the key wasn't found in the BST, or the key doesn't have a right child
    {
        while(lastParent != NULL)
        {
            if(compare(lastParent->key, key) == 0) //lastParent > root
            {
                successor = lastParent;
                break;
            }
            else
            {
...
void Predecessor(Tree *T, char key[7])
{
    ...
    while(root != NULL)
    {
        ...
        if(compare(key, root->key) == 2) //case where left subtree exists
        {
            if(root->leftChild != NULL)
            {
                ...
                flag = 1; //key is present in the BST and it has a left child
            }
            break;
        }
        else if(compare(key, root->key) == 1) //key < root
    ...
    if(flag == 0) //the key wasn't found in the BST, or the key doesn't have a right child
    {
        while(lastParent != NULL)
        {
            if(compare(lastParent->key, key) == 1) //lastParent < root
            {
                predecessor = lastParent;
                break;
            }
            else
            {
