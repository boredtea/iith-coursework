...
void direct_successor(struct Node *node, char Plate[7], char succ[7])
{
	...
	if (comparePlate(node->plate, Plate) == 2) //node->plate > plate
	{

		direct_successor(node->left, Plate, node->plate);
	}
	else if (comparePlate(node->plate, Plate) == 1) //node->plate == plate
	{
		...
		direct_successor(NULL, Plate, succ); //going to the base case
	}
	else if (comparePlate(node->plate, Plate) == 0) //node->plate < plate
	{
...
void direct_predecessor(struct Node *node, char Plate[7], char pred[7])
{
	...
	else if (comparePlate(node->plate, Plate) == 1) //node->plate == plate
	{
		if (node->left != NULL)
		{
			//finding largest value in left subtree
		...
		direct_predecessor(NULL, Plate, pred); //going to the base case
	}
	else if (comparePlate(node->plate, Plate) == 0) //node->plate < plate
	{
