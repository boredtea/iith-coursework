...
void swap(int a, int b, PQNode priorityQueue[], List *Graph[])
{
    PQNode temp;
    temp = priorityQueue[a];
    priorityQueue[a] = priorityQueue[b];
    priorityQueue[b] = temp;
    Graph[priorityQueue[a].vertex]->start->index = a;
    Graph[priorityQueue[b].vertex]->start->index = b;
}

//function to place an event in it's right position in the heap while adding
void bubbleUp(int i, PQNode priorityQueue[], List *Graph[])
{
    if ((i - 1 >= 0) && priorityQueue[(i - 1) / 2].distance > priorityQueue[i].distance)
    {
        swap(i, (i - 1) / 2, priorityQueue, Graph);
        bubbleUp((i - 1) / 2, priorityQueue, Graph);
    }
...
void addVertex(PQNode priorityQueue[], PQNode newVertex, List *Graph[])
{
    heapSize++;
    int i = heapSize - 1;
    priorityQueue[i] = newVertex;
    bubbleUp(i, priorityQueue, Graph); //finding the correct position of the new event in the heap
}
...
void percolateDown(int i, PQNode priorityQueue[], List *Graph[])
{
    int minIndex = findMinChild(i, priorityQueue);
    if (minIndex != -1 && priorityQueue[i].distance > priorityQueue[minIndex].distance)
    {
        swap(i, minIndex, priorityQueue, Graph);
        percolateDown(minIndex, priorityQueue, Graph);
    }
...
PQNode extractVertex(PQNode priorityQueue[], List *Graph[])
{
    PQNode extracted = priorityQueue[0];
    Graph[extracted.vertex]->start->index = -1;
    priorityQueue[0] = priorityQueue[heapSize - 1]; //resetting the root as the last event in the queue
    Graph[priorityQueue[0].vertex]->start->index = 0;
    
    percolateDown(0, priorityQueue, Graph);
...
void Dijkstra(List *Graph[], int numberOfVertices, int startVertex, int endVertex)
{
    ...
    addVertex(priorityQueue, newVertex, Graph);
    Node *current = Graph[startVertex]->start;

    while (heapSize > 0)
    {
        PQNode minVertex = extractVertex(priorityQueue, Graph);
