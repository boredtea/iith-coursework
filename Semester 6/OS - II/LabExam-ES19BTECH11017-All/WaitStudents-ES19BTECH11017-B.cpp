...
void testCS(int id)
{
    ...
    for (int i = 0; i < K; i++)
    {
        ...
        if(occupied_count < m - 1)
        {
            occupied_count++;
            sem_wait(&system[occupied_count]);
        }
        // entry section
        ...
        {
            occupied_count--;
            sem_post(&system[occupied_count]);
        }
...
int main()
{
    ...
    // open log file
    log_file.open("LogFile-ES19BTECH11017-B.txt", ios::out | ofstream::app);

    // initialise all semaphores to 1
    for(int i = 0; i < m; i++)
        sem_init(&(system[i]), 0, 1);
