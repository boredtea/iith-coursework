...
void writer(int id)
{
    ...
    for (int i = 0; i < kw; i++)
    {
        ...
        // entry section
        // writers code
        sem_wait(&serviceQueue); // increment
        sem_wait(&resource);     // increment
        sem_post(&serviceQueue); // decrement
...
void reader(int id)
{
    ...
    for (int i = 0; i < kw; i++)
    {
        ...
        // writers code
        sem_wait(&serviceQueue); // increment
        sem_wait(&rmutex);       // increment
        read_count++;
        if (read_count == 1)
            sem_wait(&resource); // increment
        sem_post(&serviceQueue); // decrement
        sem_post(&rmutex);       // decrement
...
int main()
{
    ...
    // open log file
    log_file = fopen("FairRW-log.txt", "w");
    ...
    fout << "Average waiting time (FRW): " << avgWaitingTime << " ms" << endl;
    fout.close();
