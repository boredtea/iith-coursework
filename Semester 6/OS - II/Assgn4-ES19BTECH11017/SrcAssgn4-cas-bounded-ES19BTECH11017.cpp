...
void testCS(int id)
{
    ...
    for (int i = 0; i < k; i++)
    {
        ...
        log_file << i + 1 << "th CS Request at " << tempTime->tm_min << ":" << tempTime->tm_sec << " by thread " << id + 1 << endl;

        // entry section
        //  critical section implementation (CAS bounded)
        bool expected = false;
        key = 1;
        waiting[id] = true;
        while(waiting[id] && key == 1)
        {
            key = !atomic_compare_exchange_strong(&lock, &expected, true);
            expected = false;
        }
        waiting[id] = false;
        ...
        gettimeofday(&elapsed, NULL);
        ms = elapsed.tv_usec / 1000.0;
        log_file << i + 1 << "th CS Entry at " << tempTime->tm_min << ":" << tempTime->tm_sec << " by thread " << id + 1 << endl;
        ...
        log_file << i + 1 << "th CS Exit at " << tempTime->tm_min << ":" << tempTime->tm_sec << " by thread " << id + 1 << endl;
...
int main()
{
    ...
    // open log file
    log_file.open("CAS-bounded-log.txt", ios::out | ofstream::app);
