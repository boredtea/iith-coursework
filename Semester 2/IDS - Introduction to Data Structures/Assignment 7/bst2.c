...
void InsertB(Tree *T, int key)
{
    ...
    {
        //pointer to find the correct "edge"
        //edge between current and one of its children is the correct edge
        current = T->start;
        ...
        /*while(1)
        {
            if(key < current->key)
            {
                if(current->leftChild != NULL && key < keycurrent->leftChild->key)
                    current = current->leftChild;
                else
                {
                    current->leftChild = newnode;
            ...
            {
                if(current->rightChild != NULL && key > keycurrent->rightChild->key)
                    current = current->rightChild;
                else
                {
                    current->rightChild = newnode;
        ...
        if(side == 1)
        {
            ...
            //sending current left pointer to the rightmost
        }
        else if(side == -1)
        {
