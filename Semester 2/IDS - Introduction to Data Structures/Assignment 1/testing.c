...
void SplitChain(Molecule *original)
{
    ...
    */
    ...
    for(int i = 0; i < (original->Alpha)->length; i++) 
    {
        ...
        newBeta->base = base;
        newBeta->next = NULL;
        Node *current = regeneratedBeta->start;
        for(int j = 0; j < i; j++)
        {
    ...
    for(int i = 0; i < (replica->Beta)->length; i++) 
    {
        ...
        printf("Current beta: %c\n", currentBeta->base);

        Node *newAlpha =  (Node*)malloc(sizeof(Node));
        newAlpha->base = base;
        newAlpha->next = NULL;
        printf("Current alpha: %c\n", newAlpha->base);
        Node *current = regeneratedAlpha->start;
        for(int j = 0; j < i; j++)
        {
            current = current->next;
        }
...
int main()
{
    ...
    do
    {
        ...
        {
        case 1:
            //case to create the first DNA molecule
            //is used only one time
            if (chain_status == -1) //checking if the chain exists
            {
            ...
            else if (chain_status != -1)
            {
                printf("ERROR: A DNA chain already exists. You can not create a new chain again.\n\n");
            }
            break;
